...
const logInfo = ["./testLogs/crashLog.txt", "UTF-8"];
const headers = { "Content-type": "application/json" };
const fileUploadUrl = "https://slack.com/api/files.upload";
const slackHook =
  "https://hooks.slack.com/services/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";
const token =
  "AZ9zd0PH5CStUOMrBlVJrHQ9";
const slackChannel = "U01991HQMNC";
...
class Reporter {
  constructor([path, coding]) {
    ...
    this.performedTestsList = this.testsPerformed
      ...
      .replace(/,/g, "\n");
    this.testType = this.file.includes("greenpath") ? "greenpath" : "redpath"
  }
      ...
      request.post(
        {
          ...
          formData: {
            ...
            file: fs.createReadStream(logInfo[0])
          }
        },
  ...
  sendSlackMessage(result) {
    ...
    (function adjustslackMessage() {
      if (result.status === "success") {
        ...
        slackMessage.attachments[0].fields.pop();
        slackMessage.attachments[0].fields[3].value = "Duration unknown";
        return slackMessage;
  ...
  getResult() {
    return new Promise((resolve, reject) => {
      if (this.testTimeNumber && !this.file.includes("FAILURES!!!")) {
        const result = {
          status: "success",
          message: `:white_check_mark:All tests completed successfully`,
          color: "#048a04",
        };
        ...
        const result = {
          status: "crashed",
          message:
            ":exclamation:Process crashed, test run failed",
          color: "#c400ad"
        };
        ...
        const result = {
          status: "failed",
          message: ":exclamation:Test run failed!",
          color: "#DC143C",
        ...
        const result = {
          status: "uncomplete",
          message:
            ":exclamation: Incomplite log file, test run failed"
        };
